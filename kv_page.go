...
type (
	KVLayout struct { // base [16]byte, offsets [size]int16, data []{...}
		BaseLayout
	}
)
...
func (l *KVLayout) pageMoveFwd(r, p []byte, i, n, di, dlen int) {
	dst := l.dataoff(p, n-1)
	dend := l.dataend(p, i)
...
again:
	...
	func() {
		...
		if ln > rn {
			...
			l.pageMoveFwd(rp, rp, 0, rn, ln-m, ds)
